...
This was completed as a Final Year Computational Physics project at Maynooth University (BSc Theoretical Physics and Pure Mathematics).

---

## Overview

At extremely high temperatures (~10⁹ K), quarks gain enough energy to escape confinement and form a quark-gluon plasma. This simulation models that phase transition using:
...
The two primary observables used to identify the phase transition are the **average plaquette** and the **Polyakov loop**, which together pin down the critical temperature to around β = 2.2, corresponding to T ≈ 235 MeV.
...
| `Average_Plaquette_Beta.py` | Average plaquette vs β for a 4³×4 lattice (reproduces Fig. 4) |
| `Thermalisation.py` | Burn-in analysis for various lattice sizes (reproduces Fig. 6) |
| `Noise_Analysis.py` | Monte Carlo noise vs lattice size — fits σ = α/√V (reproduces Fig. 7) |
| `Extrapolation.py` | Exponential extrapolation of lattice spacing a from known β values (reproduces Fig. 11) |
...
The core simulation runs the heatbath and overrelaxation algorithms over a range of β values on an 8³×4 lattice, computing the Polyakov loop expectation value and susceptibility to identify the phase transition.
...
Runs for β ∈ [1.0, 3.0] in steps of 0.1 on a 4³×4 lattice. Useful for validating the algorithm against known results and identifying the approximate location of the phase transition.
...
Tracks the average plaquette over 200 sweeps for 4³×4 and 8³×4 lattices at β = 1.9. Confirms the lattice thermalises after approximately 20 sweeps.
...
This is consistent across both the average plaquette (kink in slope around β = 2.2) and the Polyakov loop (sharp rise from ⟨P⟩ = 0 to ⟨P⟩ ≠ 0 at the same β). Monte Carlo noise was found to scale as σ = 0.2175/√V, consistent with expected lattice scaling behaviour.
...
- **Larger lattices**: e.g. 16³×4 for improved continuum limit accuracy
